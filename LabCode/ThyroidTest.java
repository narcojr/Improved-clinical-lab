...
public class ThyroidTest extends BiochemistryTest {
    ...
    public ThyroidTest(String testName, String unit,
                       double min, double max, ThyroidMarker marker) {
        super(testName, unit, min, max);   // same range for both sexes
        this.marker = marker;
    }

    // Override — first calls parent for LOW/NORMAL/HIGH, then
    // replaces with clinically meaningful thyroid labels.
    @Override
